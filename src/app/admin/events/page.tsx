...
import { events, type Event, type EventCategory, type EventStatus } from '@/lib/data';
...
const emptyForm = {
  ...
  city: '',
  venue: '',
  category: 'flagship' as EventCategory,
  capacity: '',
...
export default function AdminEventsPage() {
  ...
  function handleSave() {
    ...
    setTimeout(() => {
      setSaving(false);
      setModalOpen(false);
      setToast('Evento guardado correctamente');
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between mb-8">
        <div>
          ...
          <p className="mt-1 font-sans text-[10px] uppercase tracking-widest text-[#5B4638]">
            {events.length} eventos en total — {filtered.length} mostrados
          </p>
        </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#EAE1D6]">
              ...
              <th className="px-4 py-5 text-left font-sans text-[10px] uppercase tracking-widest text-[#5B4638] hidden xl:table-cell">Registrados/Cap</th>
                ...
                return (
                  <tr key={ev.id} className="border-b border-[#EAE1D6]/60 hover:bg-[#F7F3EE] transition-colors">
                    ...
                    <td className="px-4 py-6 hidden md:table-cell">
                      ...
                      {ev.endDate !== ev.date && (
                        <p className="font-sans text-[10px] text-[#5B4638]">– {formatDate(ev.endDate)}</p>
                      )}
                    </td>
